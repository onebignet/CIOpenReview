...
class Ad extends CI_Controller
{
	...
	function edit($id)
	{
		...
		// check form data was submitted
		if ($this->input->post('ad_submit')) {
			$data['ad'] = $this->Ad_model->getAdById($id);
			// set up form validation config
			debug('form was submitted');
			$config = array(
				array(
			...
			if ($this->form_validation->run() === FALSE) {
				...
				if ($file_error == 0) {
					...
					// update the ad
					debug('update the ad');
					$this->Ad_model->updateAd($id, $name, $orig_file_name, $remote_image_url, $height, $width, $text, $link, $visible_in_sidebar, $visible_in_lists, $visible_on_review_page);
					// reload the form
